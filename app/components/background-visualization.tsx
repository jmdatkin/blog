...
export default function BackgroundVisualization() {
  ...
  useEffect(() => {
    ...
    let width = rootElRef.current.offsetWidth,
    ...
    const light = new THREE.PointLight(0x40404a, 1000, 1000000); // soft white light
    const light2 = new THREE.PointLight(0x40404a, 10000, 100000); // soft white light
    light.position.x += 3;
    ...
    const animation: FrameRequestCallback = (time) => {
      cubes.children.forEach(
        (c, idx) =>
          (c.position.z =
            (Math.sin((time * timeScale) / 2000 + idx / 10) +
              Math.cos(idx) / 10) /
            5)
      );
      renderer.render(scene.current!, camera.current!);
    ...
    if (rootElRef.current) rootElRef.current.appendChild(renderer.domElement);
  ...
  return (
    <div
      className="w-screen h-screen fixed pointer-events-none z-[-1] theme-responsive-bg"
      ref={rootElRef}
    ></div>
  );
}
